...
/// Body rewrites for the 1.19/1.19.2 line: the 1.19.x base plus the position
/// `dismountVehicle` field and the semantic player_info/player_remove downgrade.
fn rewrite_clientbound_body_1192(canonical_id: i32, body: &[u8]) -> Option<Vec<u8>> {
    match canonical_id {
        // sync player position: appends a dismountVehicle bool.
        0x3c => {
            let mut out = Vec::with_capacity(body.len() + 1);
            out.extend_from_slice(body);
            out.push(0x00);
...
fn remap_play_clientbound(canonical_id: i32, protocol: i32) -> i32 {
    match protocol {
        ...
        // 1.19.2 folds player_remove (0x39) into the player_info packet (0x37).
        PROTO_1_19_2 if canonical_id == 0x39 => 0x37,
        PROTO_1_19_2 => apply_map(canonical_id, CB_763_TO_760),
...
/// Protocol number for Minecraft 1.19.2.
...
// Verified play packet-id map between protocol 763 (1.20.1) and 760 (1.19.2),
// from PrismarineJS minecraft-data. (Serverbound ids are identical.) Note that
// 1.19.2 merges player_remove into the single player_info packet (handled in the
// remap), so player_remove is not in this name-matched table.
const CB_763_TO_760: &[(i32, i32)] = &[
    (0x1, 0x0), (0x2, 0x1), (0x3, 0x2), (0x4, 0x3), (0x5, 0x4), (0x6, 0x5), (0x7, 0x6),
    (0x8, 0x7), (0x9, 0x8), (0xa, 0x9), (0xb, 0xa), (0xc, 0xb), (0xe, 0xd), (0xf, 0xe),
    (0x10, 0xf), (0x11, 0x10), (0x12, 0x11), (0x13, 0x12), (0x14, 0x13), (0x15, 0x14),
    (0x16, 0x15), (0x17, 0x16), (0x19, 0x18), (0x1a, 0x19), (0x1c, 0x1a), (0x1d, 0x1b),
    (0x1e, 0x1c), (0x1f, 0x1d), (0x20, 0x1e), (0x22, 0x1f), (0x23, 0x20), (0x24, 0x21),
    (0x25, 0x22), (0x26, 0x23), (0x27, 0x24), (0x28, 0x25), (0x29, 0x26), (0x2a, 0x27),
    (0x2b, 0x28), (0x2c, 0x29), (0x2d, 0x2a), (0x2e, 0x2b), (0x2f, 0x2c), (0x30, 0x2d),
    (0x31, 0x2e), (0x32, 0x2f), (0x33, 0x30), (0x34, 0x31), (0x35, 0x33), (0x36, 0x34),
    (0x37, 0x35), (0x38, 0x36), (0x3a, 0x37), (0x3b, 0x38), (0x3c, 0x39), (0x3d, 0x3a),
    (0x3e, 0x3b), (0x3f, 0x3c), (0x40, 0x3d), (0x41, 0x3e), (0x42, 0x3f), (0x43, 0x40),
    (0x44, 0x41), (0x45, 0x42), (0x46, 0x43), (0x47, 0x44), (0x48, 0x45), (0x49, 0x46),
    (0x4a, 0x47), (0x4b, 0x48), (0x4c, 0x49), (0x4d, 0x4a), (0x4e, 0x4b), (0x4f, 0x4c),
    (0x50, 0x4d), (0x51, 0x4f), (0x52, 0x50), (0x53, 0x51), (0x54, 0x52), (0x55, 0x53),
    (0x56, 0x54), (0x57, 0x55), (0x58, 0x56), (0x59, 0x57), (0x5a, 0x58), (0x5b, 0x59),
    (0x5c, 0x5a), (0x5d, 0x5b), (0x5e, 0x5c), (0x5f, 0x5d), (0x60, 0x5e), (0x61, 0x5f),
    (0x62, 0x60), (0x63, 0x61), (0x64, 0x62), (0x65, 0x63), (0x66, 0x64), (0x67, 0x65),
    (0x68, 0x66), (0x69, 0x67), (0x6a, 0x68), (0x6c, 0x69), (0x6d, 0x6a), (0x6e, 0x6b),
];

// Verified play packet-id maps between protocol 763 (1.20.1) and 759 (1.19/1.19.1),
// from PrismarineJS minecraft-data. (1.19 bodies are byte-identical to 1.19.2.)
const CB_763_TO_759: &[(i32, i32)] = &[
    (0x1, 0x0), (0x2, 0x1), (0x3, 0x2), (0x4, 0x3), (0x5, 0x4), (0x6, 0x5), (0x7, 0x6),
    (0x8, 0x7), (0x9, 0x8), (0xa, 0x9), (0xb, 0xa), (0xc, 0xb), (0xe, 0xd), (0xf, 0xe),
    (0x10, 0xf), (0x11, 0x10), (0x12, 0x11), (0x13, 0x12), (0x14, 0x13), (0x15, 0x14),
    (0x17, 0x15), (0x1a, 0x17), (0x1c, 0x18), (0x1d, 0x19), (0x1e, 0x1a), (0x1f, 0x1b),
    (0x20, 0x1c), (0x22, 0x1d), (0x23, 0x1e), (0x24, 0x1f), (0x25, 0x20), (0x26, 0x21),
    (0x27, 0x22), (0x28, 0x23), (0x29, 0x24), (0x2a, 0x25), (0x2b, 0x26), (0x2c, 0x27),
    (0x2d, 0x28), (0x2e, 0x29), (0x2f, 0x2a), (0x30, 0x2b), (0x31, 0x2c), (0x32, 0x2d),
    (0x33, 0x2e), (0x34, 0x2f), (0x35, 0x30), (0x36, 0x31), (0x37, 0x32), (0x38, 0x33),
    (0x3a, 0x34), (0x3b, 0x35), (0x3c, 0x36), (0x3d, 0x37), (0x3e, 0x38), (0x3f, 0x39),
    (0x40, 0x3a), (0x41, 0x3b), (0x42, 0x3c), (0x43, 0x3d), (0x44, 0x3e), (0x45, 0x3f),
    (0x46, 0x40), (0x47, 0x41), (0x48, 0x42), (0x49, 0x43), (0x4a, 0x44), (0x4b, 0x45),
    (0x4c, 0x46), (0x4d, 0x47), (0x4e, 0x48), (0x4f, 0x49), (0x50, 0x4a), (0x51, 0x4c),
    (0x52, 0x4d), (0x53, 0x4e), (0x54, 0x4f), (0x55, 0x50), (0x56, 0x51), (0x57, 0x52),
    (0x58, 0x53), (0x59, 0x54), (0x5a, 0x55), (0x5b, 0x56), (0x5c, 0x57), (0x5d, 0x58),
    (0x5e, 0x59), (0x5f, 0x5a), (0x60, 0x5b), (0x61, 0x5c), (0x62, 0x5d), (0x63, 0x5e),
    (0x64, 0x5f), (0x65, 0x60), (0x66, 0x61), (0x67, 0x62), (0x68, 0x63), (0x69, 0x64),
    (0x6a, 0x65), (0x6c, 0x66), (0x6d, 0x67), (0x6e, 0x68),
];

const SB_759_TO_763: &[(i32, i32)] = &[
    (0x3, 0x4), (0x4, 0x5), (0x6, 0x7), (0x7, 0x8), (0x8, 0x9), (0x9, 0xa), (0xa, 0xb),
    (0xb, 0xc), (0xc, 0xd), (0xd, 0xe), (0xe, 0xf), (0xf, 0x10), (0x10, 0x11), (0x11, 0x12),
    (0x12, 0x13), (0x13, 0x14), (0x14, 0x15), (0x15, 0x16), (0x16, 0x17), (0x17, 0x18),
    (0x18, 0x19), (0x19, 0x1a), (0x1a, 0x1b), (0x1b, 0x1c), (0x1c, 0x1d), (0x1d, 0x1e),
    (0x1e, 0x1f), (0x1f, 0x20), (0x20, 0x21), (0x21, 0x22), (0x22, 0x23), (0x23, 0x24),
    (0x24, 0x25), (0x25, 0x26), (0x26, 0x27), (0x27, 0x28), (0x28, 0x29), (0x29, 0x2a),
...
fn convert_player_info_add_763_to_old(body: &[u8], with_crypto: bool) -> Option<Vec<u8>> {
    ...
    let has_add = action & 0x01 != 0;
    let has_gm = action & 0x04 != 0;
    let has_listed = action & 0x08 != 0;
    let has_lat = action & 0x10 != 0;
    // We only translate the add form the server actually emits.
    if !has_add {
        return None;
    }
    let count = r.read_varint().ok()?;
    let mut o = BytesMut::new();
    o.write_varint(0); // 1.19.2 action enum: add_player
    o.write_varint(count);
    for _ in 0..count {
        let uuid = r.read_uuid().ok()?;
        let name = r.read_string().ok()?;
        let prop_count = r.read_varint().ok()?;
        let mut props = Vec::new();
        for _ in 0..prop_count {
            let pn = r.read_string().ok()?;
            let pv = r.read_string().ok()?;
            let sig = if r.read_bool().ok()? { Some(r.read_string().ok()?) } else { None };
            props.push((pn, pv, sig));
        }
        let gamemode = if has_gm { r.read_varint().ok()? } else { 0 };
        if has_listed {
            let _ = r.read_bool().ok()?; // 1.19.2 has no "listed" flag
        }
        let latency = if has_lat { r.read_varint().ok()? } else { 0 };
        // Emit the 1.19.2 add_player entry.
        o.write_uuid(uuid);
        o.write_string(&name);
        o.write_varint(props.len() as i32);
        for (pn, pv, sig) in &props {
            o.write_string(pn);
            o.write_string(pv);
            match sig {
                Some(s) => {
...
fn convert_player_remove_763_to_old(body: &[u8]) -> Option<Vec<u8>> {
    ...
    let count = r.read_varint().ok()?;
    let mut o = BytesMut::new();
    o.write_varint(4); // 1.19.2 action enum: remove_player
    o.write_varint(count);
    for _ in 0..count {
        o.write_uuid(r.read_uuid().ok()?);
    }
    Some(o.to_vec())
}
