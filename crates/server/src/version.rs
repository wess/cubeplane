...
fn rewrite_clientbound_body(canonical_id: i32, protocol: i32, body: &[u8]) -> Option<Vec<u8>> {
    match protocol {
        ...
        PROTO_1_19_2 => match canonical_id {
            // sync player position: 1.19.2 appends a dismountVehicle bool.
            0x3c => {
                let mut out = Vec::with_capacity(body.len() + 1);
                out.extend_from_slice(body);
                out.push(0x00);
            ...
            // player_info update / player_remove → 1.19.2's single player_info packet.
